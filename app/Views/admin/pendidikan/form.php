    ...
    <div class="container-fluid">
        ...
        <div style="background: var(--color-graphite); border: 1px solid var(--color-steel-border); border-radius: var(--radius-card); padding: 32px; margin-bottom: 24px;">
            ...
            <h3 style="color: var(--color-glacier); margin-bottom: 24px; font-family: var(--font-aeonik); font-size: 24px;">
                <?= ($action == 'add' ? 'Tambah' : 'Edit') . ' Pendidikan' ?>
            </h3>
            ...
            <form method="POST">
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <input
                        ...
                        name="periode"
                        class="form-control"
                        value="<?= htmlspecialchars($pendidikan['periode'] ?? '') ?>"
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <input
                        ...
                        name="institusi"
                        class="form-control"
                        value="<?= htmlspecialchars($pendidikan['institusi'] ?? '') ?>"
                        required
                        style="background: var(--color-void); border:1px solid var(--color-steel-border); color:var(--color-moonlight);">

                    <?php if (isset($validation) && $validation->getError('institusi')): ?>
                        <div style="color:#ff4757; font-size:12px; margin-top:6px; display:flex; align-items:center;">
                            <span style="margin-right:6px;">❌</span>
                            <?= esc($validation->getError('institusi')) ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <input
                        ...
                        name="program"
                        class="form-control"
                        value="<?= htmlspecialchars($pendidikan['program'] ?? '') ?>"
                        required
                        style="background: var(--color-void); border:1px solid var(--color-steel-border); color:var(--color-moonlight);">

                    <?php if (isset($validation) && $validation->getError('program')): ?>
                        <div style="color:#ff4757; font-size:12px; margin-top:6px; display:flex; align-items:center;">
                            <span style="margin-right:6px;">❌</span>
                            <?= esc($validation->getError('program')) ?>
                        </div>
                    <?php endif; ?>
                </div>
